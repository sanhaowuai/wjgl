...
/**
 * @类名：com.incon.framework.util.JackJson
 * @作者：Linls
 * @时间：2013-01-28
 * @版本：
 * @描述： jackjson一些转换方法
...
public class JackJson {
	 ...
	 * @return
	 */
	public static <T> T treeJsonList(JSONArray jsons, int parentId,String filterId,String filterNamePid,String filterName){
		JSONArray childMenu = new JSONArray();  
	    for (Object object : jsons) {
	    	JSONObject json = JSONObject.fromObject(object);
	    	 	int id = json.getInt(filterId);  
	    	 	int pid = json.getInt(filterNamePid);
	    	 	
	    	 	if (parentId == pid) {
	               JSONArray c_node = treeJsonList(jsons,id,filterId,filterNamePid,filterName);
	               json.put(filterName, c_node);
	               childMenu.add(json);
	           }
	    }
	    return (T) childMenu;
	}
	public static <T> T treeJsonList(JSONArray jsons, String parentId,String filterId,String filterNamePid,String filterName){
		JSONArray childMenu = new JSONArray();  
	    for (Object object : jsons) {
	    	JSONObject json = JSONObject.fromObject(object);
	    	 	String id = json.getString(filterId);  
	    	 	String pid = json.getString(filterNamePid);
