...
public class ChineseDateUtil {
	...
	private static final String[] stemNames = { "甲", "乙", "丙", "丁", "戊", "己",
			"庚", "辛", "壬", "癸" };

	private static final String[] branchNames = { "子", "丑", "寅", "卯", "辰", "巳",
			"午", "未", "申", "酉", "戌", "亥" };

	private static final String[] animalNames = { "鼠", "牛", "虎", "兔", "龙", "蛇",
			"马", "羊", "猴", "鸡", "狗", "猪" };
	...
	private static final String[] principleTermNames = { "雨水", "春分", "谷雨",
			"夏满", "夏至", "大暑", "处暑", "秋分", "霜降", "小雪", "冬至", "大寒" };

	private static final String[] sectionalTermNames = { "立春", "惊蛰", "清明",
			"立夏", "芒种", "小暑", "立秋", "白露", "寒露", "立冬", "大雪", "小寒" };

	private static final String[] monthNames = { "一", "二", "三", "四", "五", "六",
			"七", "八", "九", "十", "十一", "十二" };
	...
	public String getChineseYear() {
		String ret = stemNames[(chineseYear - 1) % 10]
	...
	public String getChineseDate() {
		...
		if (chineseMonth > 0) {
			...
			//ret = chineseDateNames[-chineseDate - 1]; --20120521修改 by yangwq
			ret = chineseDateNames[chineseDate - 1];
		}
	...
	public static void main(String[] arg) {
		...
		System.out.println(c.getChineseYear());
		System.out.println(c.getChineseMonth());
		System.out.println(c.chineseDate);
		System.out.println(c.principleTerm);
		System.out.println(c.sectionalTerm);
		System.out.println(c.getSectionalTermName());
		System.out.println(c.getPincipleTermName());
		System.out.println(c.getChineseDate());
	}

	public String outChineseYear(int y,int m,int d){
		ChineseDateUtil c = new ChineseDateUtil();
		c.setGregorian(y, m, d);
		c.computeChineseFields();
		c.computeSolarTerms();
		return c.getChineseYear();
	}
	public String outChineseMonth(int y,int m,int d){
		ChineseDateUtil c = new ChineseDateUtil();
		c.setGregorian(y, m, d);
		c.computeChineseFields();
		c.computeSolarTerms();
		return c.getChineseMonth();
	}
	public String outChineseDate(int y,int m,int d){
		ChineseDateUtil c = new ChineseDateUtil();
		c.setGregorian(y, m, d);
		c.computeChineseFields();
		c.computeSolarTerms();
		return c.getChineseDate();
	}
	public String outChineseTerm(int y,int m,int d){
		ChineseDateUtil c = new ChineseDateUtil();
		c.setGregorian(y, m, d);
		c.computeChineseFields();
		c.computeSolarTerms();
		return c.getSectionalTermName();
	}
}
