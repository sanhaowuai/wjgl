...
(function($){
	...
	$.fn.addTab = function(id, title, url, closable, selected){
		...
		var idname='tabs_' + id + '_iframe';
		$('#tabs_container').tabs('add', {
			id: id,
			title: title,
			closable: closable,
			selected: selected,
			content: '<iframe id="tabs_' + id + '_iframe" name="tabs_' + id + '_iframe" src="' + url + '" onload="IframeLoaded(\'' + id + '\');" border="0" frameborder="0" framespacing="0" marginheight="0" marginwidth="0" allowTransparency="true" style="width:100%;"></iframe>'
		});
		var iframess = document.getElementById(idname); 
		if (iframess.attachEvent){ 
		iframess.attachEvent("onload", function(){ 
			var win = iframess.contentWindow;
		    var doc = win.document;
		    var html = doc.documentElement;
		    var body = doc.body; 

		// 获取高度 
		var heightss = Math.max( body.scrollHeight, body.offsetHeight,html.clientHeight, html.scrollHeight, html.offsetHeight ); 
		var heights=heightss+110;
		heightss=heightss+50;
		document.getElementById('main_right').style.height=heights+'px';
		iframess.style.height=heightss+'px';
		}); 
		} else { 
		iframess.onload = function(){ 
