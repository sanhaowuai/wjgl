var v_DcardDataSource=function(target){
	  ...
	  var Change=function(newValue,oldValue){
		...
		this.RequestUrl=function(){
			var node=_mainObject[$(target).attr('id')]
			  return { "name":"数据来源", "group": "数据", "height":"100px", "value": ""+node['DATASOURCEURL']+"", "field": "DATASOURCEURL", 
	     ...
	     this.DataSourceResponseType=function(){
			    ...
			    var node=_mainObject[$(target).attr('id')]
				return { "name": "数据返回方式", "value": ""+node['DATASOURCERESPONSETYPE']+"", "group": "数据", "field": "position",  
					"editor": { "type": 'combobox',
				                "options":  { "valueField": 'value', 
				                	          "textField": 'name',   
				        	                   onChange:function(newValue,oldValue){
	     ...
	     this.DataSourceResult=function(){
	    	 ...
	    	 var node=_mainObject[$(target).attr('id')]
	    	 return { "name": "结果集", "value": ""+node['DATASOURCERESULT']+"", "group": "数据", "field": "datasourceresult",  
	    		 "editor": { "type": 'combobox',
