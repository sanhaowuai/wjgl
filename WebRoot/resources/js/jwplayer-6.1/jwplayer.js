if (typeof jwplayer == "undefined") {
	...
	(function(a) {
		var c = "invalid", b = "36QXq4W@GSBV^teR";
		a.key = function(h) {
			var g, i, e;
			function f(j) {
				if (!a.exists(j)) {
	...
	(function(b) {
		...
		c.encrypt = function(h, w) {
			...
			var l = c.strToLongs(a.encode(w).slice(0, 16)), i = x.length, r = x[i - 1], s = x[0], u = 2654435769, t, o, f = Math
					.floor(6 + 52 / i), m = 0;
			while (f-- > 0) {
				m += u;
				o = m >>> 2 & 3;
		...
		c.decrypt = function(j, w) {
			...
			var x = c.strToLongs(d.decode(j)), l = c.strToLongs(a.encode(w)
					.slice(0, 16)), i = x.length, r = x[i - 1], s = x[0], u = 2654435769, t, o, f = Math
					.floor(6 + 52 / i), m = f * u;
			while (m != 0) {
				o = m >>> 2 & 3;
				for ( var h = i - 1; h >= 0; h--) {
		...
		c.strToLongs = function(g) {
			...
			for ( var f = 0; f < e.length; f++) {
				e[f] = g.charCodeAt(f * 4) + (g.charCodeAt(f * 4 + 1) << 8)
						+ (g.charCodeAt(f * 4 + 2) << 16)
						+ (g.charCodeAt(f * 4 + 3) << 24)
			}
			return e
		};
		c.longsToStr = function(f) {
			...
			for ( var g = 0; g < f.length; g++) {
				e[g] = String.fromCharCode(f[g] & 255, f[g] >>> 8 & 255,
						f[g] >>> 16 & 255, f[g] >>> 24 & 255)
			}
			return e.join("")
		};
		var d = {};
		...
		d.encode = function(q, s) {
			...
			for (p = 0; p < t.length; p += 3) {
				...
				m = u >> 6 & 63;
				k = u & 63;
				l[p / 3] = h.charAt(r) + h.charAt(n) + h.charAt(m)
						+ h.charAt(k)
			}
			o = l.join("");
		...
		a.encode = function(e) {
			...
			f = f.replace(/[\u0800-\uffff]/g, function(h) {
				var g = h.charCodeAt(0);
				return String.fromCharCode(224 | g >> 12, 128 | g >> 6 & 63,
						128 | g & 63)
			});
			return f
		};
		a.decode = function(f) {
			var e = f.replace(/[\u00e0-\u00ef][\u0080-\u00bf][\u0080-\u00bf]/g,
					function(h) {
						var g = ((h.charCodeAt(0) & 15) << 12)
								| ((h.charCodeAt(1) & 63) << 6)
								| (h.charCodeAt(2) & 63);
		...
		b.plugins.pluginloader = function(i, h) {
			...
			this.setupPlugins = function(x, s, A) {
				...
				for ( var w in s.plugins) {
					var y = a.getPluginName(w), z = u[y], B = z.getFlashPath(), C = z
							.getJS(), p = z.getURL();
					if (B) {
	...
	(function(b) {
		...
		var e = b.embed = function(u) {
			var q = new e.config(u.config), j, l, h, k = q.width, m = q.height, o = "Error loading player: ", i = b.plugins
					.loadPlugins(u.id, q.plugins);
			if (q.fallbackDiv) {
				h = q.fallbackDiv;
			...
			function f() {
				if (a.typeOf(q.playlist) == "array" && q.playlist.length < 2) {
					if (q.playlist.length == 0 || !q.playlist[0].sources
							|| q.playlist[0].sources.length == 0) {
						s();
						return
					}
				}
				if (i.getStatus() == a.loaderstatus.COMPLETE) {
					for ( var x = 0; x < q.modes.length; x++) {
						if (q.modes[x].type && e[q.modes[x].type]) {
							var v = a.extend({}, q), w = new e[q.modes[x].type](
									j, q.modes[x], v, i, u);
							if (w.supportsConfig()) {
			...
			function t(v, x) {
				...
				w.display = "table";
				w.opacity = 1;
				var z = document.createElement("p"), y = z.style;
	...
	(function(c) {
		...
		var e = c.embed.flash = function(l, m, p, k, n) {
			...
			this.embed = function() {
				...
				var D = "#000000", y, u = v.wmode ? v.wmode : (v.height
						&& v.height <= 40 ? "transparent" : "opaque"), w = [
				...
				a[l.id] = v;
				if (b.isIE()) {
					var A = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" " width="100%" height="100%" id="'
	...
	(function(c) {
		var b = c.embed, a = c.utils, d = a
				.extend(
						function(k) {
							var j = (a.isHTTPS() ? "https://ssl." : "http://")
									+ "p.jwpcdn.com/"
									+ c.version.replace(/(\d+)\.(\d+).*/,
											"$1/$2/"), g = k.config, e = g.plugins, f = g.analytics, i = j
									+ "jwpsrv.js", h = new c.utils.key(c.key);
							if (h.edition() == "ads" && g.advertising) {
								g.plugins[j
										+ g.advertising.client
										+ (g.primary == "flash" ? ".swf"
												: ".js")] = g.advertising
							}
