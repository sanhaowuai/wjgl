	...
	$.fn.autoSuggest = function(data, options) {
		...
		if ((d_type == "object" && d_count > 0) || d_type == "string") {
			return this.each(function(x) {
						...
						//preFill-预填充
						var prefill_value = "";
						if (typeof opts.preFill == "string") {		//preFill为String类型
						...
						input.focus(
								...
								//.keydown(
								//源文件keydown在firefox下出现问题，改成keyup事件
								.keyup(
										function(e) {
											...
											case 8://BackSpace
												if (input.val() == "") {
													...
													if (org_li.prev().hasClass("selected")) {
														...
														if(values_input.val().indexOf(last)==0)
														{
															//在开头，则将      "XXXX,"  转成  ""
															values_input.val(values_input.val().replace("" + last + ",",""));
														}
											...
											default:
												if (opts.showResultList) {
													if (opts.selectionLimit && $("li.as-selection-item",selections_holder).length >= opts.selectionLimit) {
														results_ul
																.html('<li class="as-message">' + opts.limitText + '</li>');
														results_holder.show()
													} else {
														if (timeout) {
															clearTimeout(timeout)
						...
						function keyChange() {
							...
							if (string.length >= opts.minChars) {
								...
								if (d_type == "string") {
									...
									$.getJSON(req_string + "?" + opts.queryParam + "=" + encodeURIComponent(string) + limit + opts.extraParams,
													function(data) {
														d_count = 0;
						...
						function add_selected_item(data, num) {
							...
							var close = $('<a class="as-close">&times;</a>').click(
											function() {
												//源文件形式
