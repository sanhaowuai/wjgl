...
(function($){
	...
	function addTab(param){
	   ...
	   if(exists(param.id))
	   {
		   $('#tabs_'+param.id+'_panel', panelsContainer).remove();
		   ...
		   //closeTab(param.id);//Edit By Young 20101020//已取消，现在重新载入新连接
	       selectTab(param.id);
	       return;
	   }
	   
	   var html = '<div id="tabs_' + param.id + '"><a href="javascript:void(0);" id="tabs_link_' + param.id + '" class="tab" index="' + param.id + '" closable="' + param.closable + '" hidefocus="hidefocus">' + param.title + '</a>';
	   if(param.closable)
	      html += '<a href="javascript:void(0);" class="close" index="' + param.id + '" hidefocus="hidefocus"></a>';
	   html += '</div>';
...
//		}
	...
	function selectTab(id){
		...
		var iframe = window.frames['tabs_'+id+'_iframe'];
		//if(iframe && typeof(iframe.onactive) == 'function')//当打开无效链接时FireFox报错
		//{
		//   iframe.onactive();
		//}
		scrollTabVisible(id);
