/**
 * validatebox, timespinner: name and visible combobox, combotree, datebox &
 * datetimebox: comboname and hidden numberspinner: numberboxname and visible
...
(function($) {
				...
				box[type]({
                    ...
                    onShowPanel: function() {
                    	if(typeof($.data(dom, "loaded")) == "undefined" || $.data(dom, "loaded") == false) {
                    		...
                    		if(tri && url) {
                    			/*
								 * var val = t.find(":input[name='" + tri +
								 * "']").val();//触发器值 if(($.trim(val) == "" ||
								 * parseInt(val, 10) < 0) && ign == false) { url =
								 * null; } else { url = url + (url.indexOf("?") ==
								 * -1 ? "?" : "&") + tri + "=" + val; }
								 */
                    			var vals = [];
                ...
                $.ajax({
                    ...
                    success: function(data, status, XHR) {
                    	...
                    	if(flag){
                			...
                			window.setTimeout(function() {
                				/*
								 * var queryParams = $.data(target,
								 * "domain.datagrid").options.queryParams || {};
        ...
        $(target).datagrid($.extend({}, opts, {
        	...
        	onLoadSuccess: function(data, status, XHR) {
        		...
        		if (data && data.rows && data.rows.length > 0) {
                   ...
                   // top.$.messager.show({ title: "信息", msg: "很抱歉，没有 " + title
					// + " 。", timeout: 4000, showType: "slide" });
                }
    ...
    function changeButtonState(target) {
    	...
    	/*
		 * var t = $(target); var panel = t.datagrid("getPanel"); var length =
		 * t.datagrid("getSelections").length;
		 * panel.find(">div.datagrid-toolbar>a").each(function() { var btn =
		 * $(this); var bid = btn.attr("id"); if (bid != "btnCreate" && bid !=
		 * "btnDelete") { btn.linkbutton(length == 0 || length > 1 ? "disable" :
		 * "enable"); } else if (bid == "btnDelete") { btn.linkbutton(length ==
		 * 0 ? "disable" : "enable"); } });
		 */
    }
    /**
    ...
    $.fn.domain.defaults = {
        ...
        treegrid: $.extend({}, $.fn.treegrid.defaults, {
            ...
            idField: "id",
            /*
			 * treeField: "name", frozenColumns: [[{ field: "name", title: "名称",
			 * width: 150 }]],
			 */
            toolbar: [{
            	id: "btnCreate",
...
// }
    ...
    $.fn.domain.exp = {
    	...
    	handler:function(){
    		var t = $(this).parent().next().find(">table:hidden");
        	var opts = t.datagrid("options");
        	var frozenColumns=t.datagrid("options").frozenColumns;
        	var columns=t.datagrid("options").columns;
        	columns=frozenColumns[0].concat(columns[0]);
        	//隐藏字段
        	var hiddenColumns='';
        	//显示字段
        	var showColumns='';
        	for(var i=0;i<columns.length;i++){
        		if(columns[i].hidden==false  && columns[i].field !='opt1'){
        			showColumns=showColumns+columns[i].field+':'+columns[i].title+':'+columns[i].width+',';
        		}
        		if(columns[i].hidden==true){
        			hiddenColumns=hiddenColumns+columns[i].field+':'+columns[i].title+':'+columns[i].width+',';
        		}
        	}
        	//导出列的url
        	var end=opts.url.indexOf('page');
        	var url=opts.url.substring(0,end)+'export';
        	var src='/oa/export/index?'+$('#formQuery').serialize()+'&showColumns='+encodeURI(encodeURI(showColumns))+'&hiddenColumns='+encodeURI(encodeURI(hiddenColumns))+'&url='+url;
...
function indexQuery(options){
	var objForm = options.objForm;
     ...
     var defaultFrozenColumns = [[{field: "ck", checkbox: true}]];
     
     var frozenColumns = options.frozenColumns||defaultFrozenColumns;
