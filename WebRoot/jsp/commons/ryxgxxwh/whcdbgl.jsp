...
<script type="text/javascript" >
	...
	//查询
	function queryLike(){
	    document.queryform.action = '<%=path%>/whcdbgl/queryWhcd';
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/whcdbgl/queryWhcdByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){
			var dm = $("#dm").val();
			var zt = $("#zt").val();
			var url = "<%=path%>/whcdbgl/queryByDmCount";
		    var param={'dm':dm,'zt':zt};
		    $.post(url,param,function(result){
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/whcdbgl/addOrUpdWhcd'
	        		$('#addOrUpdfrom').ajaxSubmit(function(r){
		        		document.ff.action="<%=path%>/whcdbgl/queryWhcd";
		        		document.ff.submit();
		        	});
			    }else{
				    alert("此代码系统已存在！");
	...
	function delWhcd(){
	    ...
	    if(confirm("您确定要删除选中的选项吗？") == true){
	        document.ff.action = "<%=path%>/whcdbgl/delWhcd";
  ...
  <div class="pd10">    
    ...
    <div class="Menubox">
      <ul>
        ...
        <li class="hover"><a href="<%=path%>/whcdbgl/queryWhcd"><span>文化程度</span></a></li>
      ...
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delWhcd();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <input type="hidden" name="pageCount"  value="${whcdbPage.pageCount}" />
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" value="${whcdbPage.dmmc}"/></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
    	<input type="hidden" name="pageCount"  value="${whcdbPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${whcdbPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${whcdbPage.pageSize}" />
		<input type="hidden" name="dmmc" value="${whcdbPage.dmmc}"/><!-- 查询用 -->
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${whcdList}" var="map" varStatus="i">
    ...
    <div class="youtube">
      <c:if test="${not empty whcdbPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="whcdbPage" onlyOneShow="true" showListNo="true" action="whcdbgl/queryWhcd"/>
      </c:if>
    </div>
    <div class="clr"></div>
  </div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form name="addOrUpdfrom" id="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${whcdbPage.pageCount eq '0' ? '1' : whcdbPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${whcdbPage.pageNo eq '0' ? '1' : whcdbPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${whcdbPage.pageSize}" />
	<input type="hidden" name="dmmc" value="${whcdbPage.dmmc}"/><!-- 查询用 -->
