...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/gjgl/queryGjgl";
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			...
			document.addOrUpdfrom.action="<%=path%>/gjgl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/gjgl/addGjglSz';
		        	$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.ff.action="<%=path%>/gjgl/queryGjgl";
						document.ff.submit();
					});
			    }else{
				    alert("此代码已存在！");
	...
	function delGj(){
	    ...
	    if(confirm("您确定要删除选中的国籍吗？") == true){
	        document.ff.action = "<%=path%>/gjgl/delGj";
  ...
  <div class="pd10">    
    ...
    <div class="Menubox">
      <ul>           
        ...
        <li class="hover"><a href="<%=path%>/gjgl/queryGjgl"><span>国籍</span></a></li>
      </ul>
    </div> 
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delGj();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${gjglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
	    <input type="hidden" name="pageCount"  value="${gjglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${gjglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${gjglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${gjglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${gjList}" var="map" varStatus="status">
    ...
    <div class="youtube"> 
      <c:if test="${not empty gjglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="gjglPage" onlyOneShow="true" showListNo="true" action="gjgl/queryGjgl"/>
      </c:if>
    </div>
    <div class="clr"></div>
  </div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${gjglPage.pageCount eq '0' ? '1' : gjglPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${gjglPage.pageNo eq '0' ? '1' : gjglPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${gjglPage.pageSize}" />
	<input type="hidden" name="dmmc"  value="${gjglPage.dmmc}" />
    ...
    <div class="popup">
      ...
      <table id="divtab" width="100%"  cellpadding="0" cellspacing="0"  class="addlist" style="border-collapse:collapse">
        ...
        <tr>
         ...
         <tr>
            <th>简称：</th>
            <td align="left"><input type="text" name="jc" id="ff_jc" /></td>
        </tr>
        <tr>
          <th>是否可用：</th>
          <td align="left">
