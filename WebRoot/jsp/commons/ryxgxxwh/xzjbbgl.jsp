...
<script type="text/javascript" >
	...
	//查询
	function queryLike(){
	    document.queryform.action = '<%=path%>/xzjbbgl/queryXzjb';
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/xzjbbgl/queryXzjbByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){
			...
			document.addOrUpdfrom.action= "<%=path%>/xzjbbgl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/xzjbbgl/addOrUpdXzjb'
	        		$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.ff.action="<%=path%>/xzjbbgl/queryXzjb";
						document.ff.submit();
					});
			    }else{
				    alert("此代码系统已存在！");
	...
	function delXzjb(){
	    ...
	    if(confirm("您确定要删除选中的选项吗？") == true){
	        document.ff.action = "<%=path%>/xzjbbgl/delXzjb";
  ...
  <div class="pd10">    
    ...
    <div class="Menubox">
      <ul>
        ...
        <li class="hover"><a href="<%=path%>/xzjbbgl/queryXzjb"><span>行政级别</span></a></li>
      ...
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delXzjb();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <input type="hidden" name="pageCount"  value="${xzjbbPage.pageCount}" />
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" value="${xzjbbPage.dmmc}"/></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
    	<input type="hidden" name="pageCount"  value="${xzjbbPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${xzjbbPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${xzjbbPage.pageSize}" />
		<input type="hidden" name="dmmc" value="${xzjbbPage.dmmc}"/><!-- 查询用 -->
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${xzjbList}" var="map" varStatus="i">
    ...
    <div class="youtube">
      <c:if test="${not empty xzjbbPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="xzjbbPage" onlyOneShow="true" showListNo="true" action="xzjbbgl/queryXzjb"/>
      </c:if>
    </div>
    <div class="clr"></div>
  </div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form name="addOrUpdfrom" id="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${xzjbbPage.pageCount eq '0' ? '1' : xzjbbPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${xzjbbPage.pageNo eq '0' ? '1' : xzjbbPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${xzjbbPage.pageSize}" />
	<input type="hidden" name="dmmc" value="${xzjbbPage.dmmc}"/><!-- 查询用 -->
