...
<script type="text/javascript" >
	...
	//查询
	function queryLike(){
	    document.queryform.action = '<%=path%>/xwbgl/queryXw';
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/xwbgl/queryXwByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){
			...
			document.addOrUpdfrom.action= "<%=path%>/xwbgl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/xwbgl/addOrUpdXw'
	        		$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.ff.action="<%=path%>/xwbgl/queryXw";
						document.ff.submit();
					});
			    }else{
				    alert("此代码系统已存在！");
	...
	function delXw(){
	    ...
	    if(confirm("您确定要删除选中的选项吗？") == true){
	        document.ff.action = "<%=path%>/xwbgl/delXw";
    ...
    <div class="pd10">    
      ...
      <div class="Menubox">
        <ul>
          ...
          <li><a href="<%=path%>/xlbgl/queryXl"><span>学历</span></a></li>
          <li><a href="<%=path%>/xwlbbgl/queryXwlb"><span>学位类别</span></a></li>
          <li class="hover"><a href="<%=path%>/xwbgl/queryXw"><span>学位</span></a></li>
        ...
        <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
          <tr>
            <td height="35px">
              ...
              <div class="addlist_button  ml15"> <a href="javascript:delXw();"><span>删&nbsp;&nbsp;除</span></a> </div>
            </td>
          </tr>
        </table>
        <form id="queryform" name="queryform" method="post">
          <input type="hidden" name="pageCount"  value="${xwbPage.pageCount}" />
          <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
            <tr>
              <td >名称：</td>
              <td><input type="text" name="dmmc" value="${xwbPage.dmmc}"/></td>
      ...
      <div class="list">
        <form name="ff" method="post" id="ff" >
      	  <input type="hidden" name="pageCount"  value="${xwbPage.pageCount}" />
		  <input type="hidden" name="pageNo"  value="${xwbPage.pageNo}" />
		  <input type="hidden" name="pageSize"  value="${xwbPage.pageSize}" />
		  <input type="hidden" name="dmmc" value="${xwbPage.dmmc}"/><!-- 查询用 -->
          <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
            ...
            <c:forEach items="${xwList}" var="map" varStatus="i">
      ...
      <div class="youtube">
      	<c:if test="${not empty xwbPage }">
          <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="xwbPage" onlyOneShow="true" showListNo="true" action="xwbgl/queryXw"/>
        </c:if>
      </div>
      <div class="clr"></div>
    </div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form name="addOrUpdfrom" id="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${xwbPage.pageCount eq '0' ? '1' : xwbPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${xwbPage.pageNo eq '0' ? '1' : xwbPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${xwbPage.pageSize}" />
	<input type="hidden" name="dmmc" value="${xwbPage.dmmc}"/><!-- 查询用 -->
    ...
    <div class="popup">
      ...
      <table id="divtab" width="100%" cellpadding="0" cellspacing="0" class="addlist" style="border-collapse:collapse">
        ...
        <tr>
          <th>学生类别：</th>
          <td align="left">
            <select name="xslbdm" id="xslbdm"  class="se150" dataType="Require" msg="请选择学生类别">
         	  <option value="">--请选择学生类别--</option>
          	  <c:forEach items="${xwlbList}" var="map" varStatus="status">
          	    <option value='${map.dm}'>
       	          ${map.mc}
      	        </option>
    	      </c:forEach>
