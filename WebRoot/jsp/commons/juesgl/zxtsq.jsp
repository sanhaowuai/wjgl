...
<script type="text/javascript">	
	...
	//查询
	function queryLike(){
	    document.queryform.action = '<%=path%>/zxtwh/queryZxtqx';
	    document.queryform.submit();
	}
	//添加组件维护权限
	function addZjwhQx(){		
	    if(confirm("您确定选中要授权的组件吗？") == true){
	        document.ff.action = '<%=path%>/zxtwh/addJsZxtsq';
	        $('#ff').ajaxSubmit(function(r){
	    		if(r="1"){
					alert("授权成功!");					
					parent.window.BOX_remove('qx_zxt');
...
<%--      <input type="hidden" name="qxlb"    value="${qxlb}"/>	--%>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
      	<input type="hidden" name="jsdm"   value="${jsdm}"/>		
      	<input type="hidden" name="pxtmc" value="${page.pxtmc}"/>
		<!-- 查询用 -->
...
<%--        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="page" onlyOneShow="true" showListNo="true" action="zjwh/queryZjqx"/>--%>
<%--      </c:if>--%>
<%--    </div>--%>
    <div class="clr"></div>
