...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/xsgl/queryXsgl";
	...
	function updDiv(xh){
	    ...
	    $.post(url,param,function(result){
	        ...
	        $("#ff_nj").val(null == result['xsxxbEntity']['nj'] ? "" : result['xsxxbEntity']['nj']);
	       // var abc=""+result['xsxxbEntity']['sjlj']+result['xsxxbEntity']['bszplj']; 
	       var abc="<%=path%>/"+result['xsxxbEntity']['bszplj']; 
	...
	function delXs(){
	    ...
	    if(confirm("您确定要删除选中的学生吗？") == true){
	        document.ff.action = "<%=path%>/xsgl/delXs";
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){
			...
			var url="<%=path%>/xsgl/queryByXhCount";
		    var param={'xh':xh,'zt':zt};
		    var xb = $("input[name=xbs]:checked").attr("id");
		    $("#xb").val(xb);
		    $.post(url,param,function(result){
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/xsgl/addOrUpdXs';	    
	...
	function changeX(obj){
		var xydm = $(obj).val();
		var url1 = "<%=path%>/xsgl/querySfglbm";
		   $.post(url1,{'yxdm':xydm},function(result){
		       if(result == '1'){
		       	   $("#glyx").val(xydm);
		       }
	...
	//导出模版
	function dcmb(){
		document.queryform.action = "<%=path%>/xsgl/excelDcmb";
        document.queryform.submit();
	}
	//导入模版
	function drmb(){
		if(Validator.Validate(document.getElementById('drform'),3)){
		    document.drform.action = "<%=path%>/xsgl/impPyfa";
...
<!--		<input type="hidden" name="xsxh"  value="${xsglPage.xsxh}" />-->
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${xslist}" var="map" varStatus="status">
			  ...
			  <td>${map.zzmmmc }</td>
	          <td>
	            <div class="cz_button cz1_1" title="修改" onclick="javascript:updDiv('${map.xh}');"></div>
    ...
    <div class="youtube">
        ...
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="xsglPage" onlyOneShow="true" showListNo="true" action="xsgl/queryXsgl"/>
      </c:if>
    </div>
    <div class="clr"></div>
  </div>
  ...
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
    ...
    <div class="popup">
      ...
      <table id="divtab" width="100%"  cellpadding="0" cellspacing="0"  class="addlist" style="border-collapse:collapse">
        <tr>
          <th>学号：</th>
          <td align="left"><input type="text" name="xh" id="xh" dataType="Limit" min="1" max="20" msg="必填(20个字之内)"/>
...
<!-- 导入模板 -->
<form name="drform" id="drform"  method="post"  enctype="multipart/form-data">
...
<!--         </tr> -->
        <tr>
          <th width="100">模板下载：</th>
          <td align="left"><a href="javascript:void(0);" onclick="javaScript:dcmb();" title="点击下载">学生模版下载</a></td>
        </tr>
        <tr>
          <th valign="top">文本选择：</th>
          <td align="left">
        ...
        <tr>
          ...
          <div class="addlist_button1 mr15" style="float:right" > <a href="javascript:drmb();" title="保存" ><span>保存</span></a> </div></td>
        </tr>
      </table>
      <div class="clr"></div>
    </div>
