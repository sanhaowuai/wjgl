...
<style type="text/css">
  ...
  *html .Contentbox { height:100% }
  ...
  <script type="text/javascript" >
	function queryLike(){
		document.queryform.action="<%=path%>/gzlshywlc/queryShywlcyxjdPage";
	...
	function todbsh(dbjddm,ywlcdm,ywlcslid,shxqlj){
		$('#dbjddm').val(dbjddm);
		$('#ywlcdm').val(ywlcdm);
		$('#ywlcslid').val(ywlcslid);
		if(shxqlj==null || shxqlj==''){//业务流程审核查看链接，如果为空，默认公共页面
			document.queryform.action="<%=path%>/gzlshywlc/queryShywlcTodbsh";
			document.queryform.submit();
		
		}else{//不为空，走配置
			document.queryform.action="<%=path%>/"+shxqlj;
			document.queryform.submit();
		}
...
<div class="Contentbox">
  <div class="pd10">    
	 ...
	 <div class="dropdown pull-right">
       <form name="queryform" method="post" id="queryform" >
       	...
       	<input type="hidden" name="ywlcslid" id="ywlcslid"/>
       		<input type="hidden" name="dbjddm" id="dbjddm"/>
	   ...
	   <div class="btn-group">
          ...
          <table border="0" width="96%"  cellpadding="0" cellspacing="0"  >
		      <tr>
		      	<th width="120">标题：</th>
	           	<td> <input type="text" name="pageBt" id="pageBt" value="${gzlShywlcyxjdPage.pageBt}"  /></td>
		       </tr>
		       <tr>
		        <th>业务名称：</th>
	           	<td><input type="text" name="pageYwlcgjz" id="pageYwlcgjz" value="${gzlShywlcyxjdPage.pageYwlcgjz}"  /></td>
		      </tr>
		      <tr>
	           	...
	           	<td>
	           	  <select name="pageYwlclbdm" id="pageYwlclbdm"  >
	      		    ...
	      		    <c:forEach items="${ywlclbList}" var="map" varStatus="status">
	       		      <option value='${map.DM}' <c:if test="${map.DM eq gzlShywlcyxjdPage.pageYwlclbdm}">selected</c:if>>
	    		         ${map.MC}
	    	          </option>
				    </c:forEach>
	    		  </select>
        ...
        <table  class="table table-bordered bot_line">
         ...
         <c:forEach items="${list}" var="a">
         	<tr>
	            ...
	            <td>${a.yshjdztxsmc }</td>
	            <td>
	            <div  onclick="javascript:todbsh('${a.dbjddm}','${a.ywlcdm}','${a.ywlcslid}','${a.shxqlj}');" class="cz_button cz_bg1"><span class="glyphicon glyphicon-pencil"></span></div>
    ...
    <div class="youtube"> 
        <c:if test="${not empty gzlShywlcyxjdPage }">
            <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="gzlShywlcyxjdPage" onlyOneShow="true" showListNo="true" action="gzlshywlc/queryShywlcyxjdPage"/>
        </c:if>
    </div>
    <div class="clr"></div>
  </div>
