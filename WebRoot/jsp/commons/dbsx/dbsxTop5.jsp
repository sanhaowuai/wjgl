...
<script type="text/javascript" >
	...
	{
					document.ff.action="<%=path%>/dbsx/querySfkck?id="+id;
					$("#ff").ajaxSubmit(function(data){
				      if(data==1)
				      {
				      alert("该事项已被读取");
...
<!--      <a id="a_id"  target="${map.dkfs}" onclick="javascript:tiaozhuan('${map.ljdz}','${map.id}');" href="${map.ljdz}">${map.bt}</a>-->
         <a id="a_id"  target="${map.dkfs}" onclick="javascript:tiaozhuan();" href="<%=path %>/dbsx/updSee?id=${map.id}&href=${map.ljdz}" >${map.bt}</a>
          </c:if>
          <c:if test="${map.sfylj ne '1'}">
          <a  href="javascript:void(0);" onclick="javascript:queryById('${map.id}');" title="${map.bt}">${map.bt}</a>
          </c:if> 
