...
<script src="<%=path%>/resources/style/blueEight/js/bottomMenuTwo.js"></script>
<script src="<%=path%>/resources/style/blueEight/js/index_13.js?asa=127" type="text/javascript"></script> 
...
$(function(){
	...
	$('#toggle').click(function(){
		if(isHidden == 0){//缩小
			...
			isHidden = 0;
			$('#frmTitle').animate({"width":"250px","padding-left":"0"},100);
...
$(function (){
	$(".main_left").mouseout(function (){
		$(window.iframename0.document).scrollTop(0);
		setTimeout(function (){
			$(window.iframename0.document).scrollTop(0);
			},602 );
		});
...
function colRwLoc(){
	$('.tabs-panel').each(function (){
		var s=$(this).children('iframe').attr("src");
		if($(this).hasClass("selected")){
			}else{
		$(this).children('iframe').attr("src",s);
...
background-color:rgba(47, 105, 162, 0.8);
}
#myiframes{
width:100%!important;
}
...
<body style="height:100%; zoom:1;background:#fff" onload="init();">
...
<div class="top" id="north" style="background:url('<%=path%>/resources/style/blueEight/images/header-bg.png') no-repeat center;background-size:cover;">
   <div class="topbg">
	<div class="logo">${applicationScope.XT_CSB_TITLE.CS1}</div>
	<div class="top_right">
		<div id="taskbar_right">
		<c:forEach items="${listxx}" var="map">
			<c:choose>
		   		<c:when test="${map.id eq '1'}">  <!-- 主页  -->
			   	 <a style="display:none;" class="pngFix" href="javascript:getReturn()"; onmouseover="toShow('home');" onmouseout="toHidden('home')"  id="home">
				 <i class="${map.icon}"></i><span>${map.ms}</span></a> 
	              ...
	              <div class="top-inner">${map.ms} </div>
		   		...
		   		<a style='padding-top:12px;' class="pngFix" href="javascript:BOX_show('updMm');" onmouseover="toShow('unlock');" onmouseout="toHidden('unlock')"  id="unlock">
		 		<i class="${map.icon}"></i><span>${map.ms}</span></a> 
	              ...
	              <div class="top-inner">${map.ms} </div>
		   		...
		   		<a style='padding-top:12px;' class="pngFix" href="javascript:logout();" onmouseover="toShow('power');" onmouseout="toHidden('power')"  id="power">
		 		<i class="${map.icon}"></i><span>${map.ms}</span></a> 
	          <div class="toptip"  id="power_a" style="display:none">
	              ...
	              <div class="top-inner">${map.ms} </div>
		   		...
		   		<a class="pngFix" style='padding-top:3px;' href="javascript:void(0);" onclick="javascript:createTab('uuu5555555','个人菜单维护','<%=path%>	/grb/queryGrbcd');"  onmouseover="toShow('grwh');" onmouseout="toHidden('grwh')"  id="grwh">
		 		<i class="${map.icon}"></i><span>${map.ms}</span></a> 
	          <div class="toptip"  id="grwh_a" style="display:none">
	              ...
	              <div class="top-inner">${map.ms}</div>
		   		...
		   		<a class="pngFix" style='padding-top:12px;' href="javascript:void(0);" onclick="javascript:zybjsz()" onmouseover="toShow('tooltitle');" onmouseout="toHidden('tooltitle')"  id="tooltitle">
		 		<i class="${map.icon}"></i><span>${map.ms}</span></a>
	          <div class="toptip"  id="tooltitle_a" style="display:none">
	              ...
	              <div class="top-inner">${map.ms} </div>
		   		...
		   		<c:when test="${map.id eq '6'}">  <!-- 角色 -->
				   	 	<div id="site-nav">
						<div class="switch">
							<div class="photo"><a class="pngFix" style='padding-top:12px;' title="${map.ms}" href="javaScript:void(0);"><i class="${map.icon}"></i><span>${map.ms}</span></a></div>
							<div class="switch-bd">
		   		...
		   		<a class="pngFix" style='padding-top:3px;' href="javascript:void(0);" onclick="javascript:top.closeAllTab();" onmouseover="toShow('th');" onmouseout="toHidden('th')"  id="th">
		 		<i class="${map.icon}"></i><span>${map.ms}</span></a> 
	...
	<!--		   		 <a title="${map.ms}" class="pngFix"  href="javascript:void(0);" onclick="javascript:iframe_div('${map.fwlj}','${map.ms}','${map.nww}')"  style="background:#FFA200"><i class="${map.icon}"></i></a>	-->
		...
		<!--		   			<a title="${map.ms}" class="pngFix"   href="${map.fwlj}" target="${map.tcfs}"  style="background:#FFA200"><i class="${map.icon}"></i></a>	-->
			...
			<a  class="pngFix fhTop" onclick="fhdbUp()" style='display:none;'>
						 		<i class="fa fa-chevron-up"></i><span>返回顶部</span></a>
		</div>
...
<div id="tabs_portal_0_panel">
	<div class="main_left"  name="fmTitle" id="frmTitle">
		<div id="skin-select">
			<div class="skin-dinwei" id="skin-dinwei"><i class="fa fa-caret-left"></i></div>
 ...
 <div  id="main_right" class="main_right"  >
    <div id="myiframes">
	    ...
	    <div  id="center" class="container-fluid paper-wrap bevel tlbr">
			<div id="tabs_portal_0_panel"  class="tabs-panel selected index-vi"> 
		    	<iframe id="tabs_portal_0_iframe"  name="tabs_portal_0_iframe" src="<%=path%>/jsp/commons/main/right.jsp" scrolling='no' border="0" framespacing="0" marginheight="0" marginwidth="0" frameborder="0" allowTransparency="true"></iframe>
		    </div>  
		</div>
	</div>
    <div id="cdbutton" style="display:none"></div>
		...
		<a id="qhcd" style='padding-top:12px;' href="javascript:cdbd();" class="switch" style="z-index:9;position:relative;"> 切换   </a>
...
<form name="divFrom" id="divFrom" method="post">
    <div class="popup">
        <div class="Floatleft Pct100 mb10">
            ...
            <a style='padding-top:12px;' href="#" class="btn_close" onclick="javascript:BOX_remove('updMm');"></a> </div>
        <div class="clr"></div>
