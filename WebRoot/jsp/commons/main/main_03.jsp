...
<script type="text/javascript" src="<%=path%>/resources/js/common/menu/jquery-1.6.4.min.js"></script>
<script type="text/javascript" src="<%=path%>/resources/js/common/menu/DD_belatedPNG.js"></script>
<script type="text/javascript" src="<%=path%>/resources/js/common/validator.js"></script> 
<script type="text/javascript">
	$(document).ready(function(){ 
		$('.menu08 li').hover(function(){
		$(this).children('ul.one').css({left:$(this).offset().left}); 
...
function changeClass(id){
	$("#"+id).removeClass().addClass("more");
}

function changeSrc(srcRight){ 
	document.getElementById("iframename").src=srcRight;
} 
</script>
</head>
...
<div class="topbg">
	...
	<div class="top_right">
		<div id="taskbar_right">
			<div id="site-nav">
				<div class="switch">
					<div class="photo"><a  id="role" class="pngFix" title="角色" href="#"></a></div>
					<div class="switch-bd" >
		...
		<a id="logout" title="注销登录" class="pngFix" href="javascript:logout();"></a> 
		</div>
        ...
        <div id="tabs_container"> 
		  <ul class="menu08">
		  	<c:forEach items="${orderListFather}" var="fmap" varStatus="i">
			    <li><a href="javascript:void(0);">${fmap.QXMC}</a>
			      <ul class="one">
			      	<c:forEach items="${orderList}" var="zmap">
			      		<c:if test="${fmap.QXDM eq zmap.FQXDM}">
			      		 	<li><a href="<%=path%>/${zmap.FWLJ}" target="iframename" id="more_${zmap.QXDM}">${zmap.QXMC}</a>
			      		 		<ul class="two">
			      		 			<c:forEach items="${orderList}" var="two">
			      						<c:if test="${zmap.QXDM eq two.FQXDM}">
			      							<script>changeClass('more_${zmap.QXDM}')</script>
			      		 		 		<li><a href="<%=path%>/${zmap.FWLJ}" target="iframename">${two.QXMC}</a></li>
			      		 		 		</c:if>
			      		 		 	</c:forEach>
			      		 		</ul>
			      		 	</li>
