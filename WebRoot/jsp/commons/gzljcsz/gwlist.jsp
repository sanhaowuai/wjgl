...
function querylike(){
	document.queryform.action="<%=path%>/ywlcsz/queryAllgw";
    ...
    <div class="pd10">
        <div class="butsea">
            ...
            <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
                <tr>
                    <td >岗位代码/名称：</td>
                    <td><input type="text" name="gwmc" id="gwmc" value="${gwPage.gwmc}"  /></td>
        ...
        <form name="ff" method="post" id="ff" >
		    ...
		    <c:forEach items="${list}" var="map"   varStatus="status">
	        	...
	        	<td ><a class="select_btn" href="javascript:void(0);" onclick='javascript:resultjsgl("${map.GWDM},${map.GWMC}")';>选择</a></td>
	         	<td >${map.GWDM}</td>
	         	<td >${map.GWMC}</td>
	       		</tr>
      		</c:forEach>
        ...
        <div class="youtube">
        <c:if test="${not empty gwPage }">
           <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="gwPage" onlyOneShow="true" showListNo="false" action="ywlcsz/queryAllgw"/>
          </c:if>
        </div>
    </div>
</div>
