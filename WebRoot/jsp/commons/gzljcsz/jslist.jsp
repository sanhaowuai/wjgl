...
function querylike(){
	document.queryform.action="<%=path%>/ywlcsz/queryAllJs";
    ...
    <div class="pd10">
        <div class="butsea">
            ...
            <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
                <tr>
                    <td >角色代码/名称：</td>
                    <td><input type="text" name="jsmc" id="qf_jsmc" value="${jueSePage.jsmc}"  /></td>
        ...
        <form name="ff" method="post" id="ff" >
		    ...
		    <c:forEach items="${list}" var="map"   varStatus="status">
	        	...
	        	<td ><a class="select_btn" href="javascript:void(0);" onclick='javascript:resultjsgl("${map.JSDM},${map.JSMC}")';>选择</a></td>
	         	<td >${map.JSDM}</td>
	         	<td >${map.JSMC}</td>
	       		</tr>
      		</c:forEach>
        ...
        <div class="youtube">
        <c:if test="${not empty jueSePage }">
           <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="jueSePage" onlyOneShow="true" showListNo="false" action="ywlcsz/queryAllJs"/>
          </c:if>
        </div>
    </div>
</div>
