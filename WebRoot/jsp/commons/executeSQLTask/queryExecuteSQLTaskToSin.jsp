...
function shumitExecuteSQLTaskToSin(){
	if(Validator.Validate(document.getElementById('ff'),3) ){
		if(confirm("您确定要保存么？") == true){
			$("#ff_id").removeAttr("disabled");
			$("#ff").ajaxSubmit({
			     type: "post",
			     url: "<%=path%>/executeSQLTaskController/shumitExecuteSQLTaskToSin/"+$("#ff_method").val(),
			     dataType: "json",
...
function addJob(vid){
	if(Validator.Validate(document.getElementById('ff'),3) ){
		if(confirm("您确定要启动么？") == true){
			$("#ff").ajaxSubmit({
			     type: "post",
			     url: "<%=path%>/executeSQLTaskController/addJob/"+vid,
			     //dataType: "json",
			     success: function(result){
						if(result== "success"){
							alert("操作成功");
...
<div class="pd10">
    ...
    <div class="butsea">
  		 ...
  		 <a  href="javascript:void(0);" onclick="javascript:shumitExecuteSQLTaskToSin();return false;"><span style="font-size:12px;">保&nbsp;&nbsp;存</span></a> 
<%--   		 <c:if test="${method eq 'upd'}"> --%>
<%--   		 <a  href="javascript:void(0);" onclick="javascript:addJob('${ExecuteSQLTaskEntitySin.id}');return false;"><span style="font-size:12px;">运行</span></a>  --%>
<%--   		 </c:if> --%>
		      ...
		      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse"  class="addlist"  >
		        <tr>
		          ...
		          <td colspan="3">
		            ...
		            <input type="text" id="ff_id" class="form-control pull-left" name="id" value="${ExecuteSQLTaskEntitySin.id}" <c:if test="${method eq 'upd'}"> readonly="readonly"</c:if>maxlength=36 dataType="Limit" min="1" max="100" msg="请填写ID"/>
		         ...
		         <tr>
		          ...
		          <td width="40%" colspan="3">
		            <input type="text" name="mc"  value="${ExecuteSQLTaskEntitySin.mc}"/>
		          </td>
		        </tr>
		        <tr>
		          <th width="10%">描述：</th>
		          <td width="40%" colspan="3">
					<textarea rows="" cols="" name = 'ms'>${ExecuteSQLTaskEntitySin.ms}</textarea>
		          </td>
		        </tr>
		        <tr>
		          <th width="10%">处理SQL：</th>
		          <td width="40%" colspan="3" >
					<textarea rows="10" cols="200" name = 'clsql'>${ExecuteSQLTaskEntitySin.clsql}</textarea>
		          </td>
		        </tr>
		        <tr>
		          <th width="10%">执行频率：</th>
		          <td width="40%" colspan="3">
<input id="cron" class="form-control pull-left" id="cron" name="cron" value="${ExecuteSQLTaskEntitySin.cron}" readonly="readonly"/>
 <div class="addlist_button">
 <a  href="javascript:void(0);" 
 onclick="javascript:cron_div_open('${ExecuteSQLTaskEntitySin.cron}');return false;"><span style="font-size:12px;">设&nbsp;&nbsp;置</span></a> 
 </div>
