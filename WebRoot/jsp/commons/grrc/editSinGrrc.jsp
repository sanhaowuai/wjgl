...
<div class="popup">
 <div class="popup_title">
    ...
    <table width="100%"  cellpadding="0" cellspacing="0"  class="addlist" style="border-collapse:collapse">
     <tr>
       <td>
         ...
         <!-- 操作标识（0：个人；1：系统推送） -->
         <c:choose>
         	<c:when test="${sinGrrc.czbs eq '0'}">
         	<input name="rcsj" type="hidden" value="${sinGrrc.rcsj}"/>
         	<textarea rows="10" cols="43" name="title"  style="resize:none">${sinGrrc.title}</textarea>
         	</c:when>
         	<c:otherwise><p>${sinGrrc.title}</p></c:otherwise>
         </c:choose>
         </form>
       </td>
     </tr>
   ...
   <c:if test="${sinGrrc.czbs eq '0'}">
        <div class="addlist_button2 ml10" style="float:right"><a onclick="delSinGrrc('${sinGrrc.id}')" href="javascript:void(0);"><span>删除</span></a></div>   
        <div class="addlist_button ml10" style="float:right"><a onclick="updShumitSinGrrc('${sinGrrc.id}')" href="javascript:void(0);"><span>保存</span></a></div>
   </c:if> 
