...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/zylbgl/queryZylbgl";
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/zylbgl/queryZylbByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			var dm = $("#ff_dm").val();
			var zt = $("#zt").val();
			var url="<%=path%>/zylbgl/queryByDmCount";
		    var param={'dm':dm,'zt':zt};
		    $.post(url,param,function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/zylbgl/addZylbglSz';
		        	$('#addOrUpdfrom').ajaxSubmit(function(r){
		        		document.ff.action="<%=path%>/zylbgl/queryZylbgl";
		        		document.ff.submit();
		        	});
			    }else{
				    alert("此代码已存在！");
	...
	function delZylb(){
	    ...
	    if(confirm("您确定要删除选中的专业类别吗？") == true){
	        document.ff.action = "<%=path%>/zylbgl/delZylb";
...
<!--          </div> -->
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delZylb();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${zylbglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${zylbglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${zylbglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${zylbglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${zylbglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
		  ...
		  <c:forEach items="${zylbList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
      <c:if test="${not empty zylbglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="zylbglPage" onlyOneShow="true" showListNo="true" action="zylbgl/queryZylbgl"/>
      </c:if>
    </div>
    <div class="clr"></div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${zylbglPage.pageCount eq '0' ? '1' : zylbglPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${zylbglPage.pageNo eq '0' ? '1' : zylbglPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${zylbglPage.pageSize}" />
	<input type="hidden" name="dmmc"  value="${zylbglPage.dmmc}" />
