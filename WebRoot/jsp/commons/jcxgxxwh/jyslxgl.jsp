...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/jyslxgl/queryJyslxgl";
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/jyslxgl/queryJyslxByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			...
			document.addOrUpdfrom.action="<%=path%>/jyslxgl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/jyslxgl/addJyslxglSz';
		        	$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.addOrUpdfrom.action="<%=path%>/jyslxgl/queryJyslxgl";
	...
	function delJyslx(){
	    ...
	    if(confirm("您确定要删除选中的教研室类型吗？") == true){
	    	var url = "<%=path%>/jyslxgl/queryExit";
	    	...
	    	$.post(url,zhi,function(result){
	    		if(result.mc == ""){
	    			document.ff.action = "<%=path%>/jyslxgl/delJyslx";
	    			$("#ff").ajaxSubmit(function(r){
						document.ff.action="<%=path%>/jyslxgl/queryJyslxgl";
						document.ff.submit();
					});
	    		}else{
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delJyslx();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${jyslxglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${jyslxglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${jyslxglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${jyslxglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${jyslxglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
		  ...
		  <c:forEach items="${jyslxList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
      <c:if test="${not empty jyslxglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="jyslxglPage" onlyOneShow="true" showListNo="true" action="jyslxgl/queryJyslxgl"/>
      </c:if>
    </div>
    <div class="clr"></div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${jyslxglPage.pageCount eq '0' ? '1' : jyslxglPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${jyslxglPage.pageNo eq '0' ? '1' : jyslxglPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${jyslxglPage.pageSize}" />
	<input type="hidden" name="dmmc"  value="${jyslxglPage.dmmc}" />
