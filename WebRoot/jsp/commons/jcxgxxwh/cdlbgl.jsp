...
$(document).ready(function(){
   $("#ff").baigoCheckall();
	...
	function querylike(){
		document.queryform.action="<%=path%>/cdlbgl/queryCdlbgl";
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/cdlbgl/queryCdlbByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			var dm = $("#ff_dm").val();
			var zt = $("#zt").val();
			var url="<%=path%>/cdlbgl/queryByDmCount";
		    var param={'dm':dm,'zt':zt};
		    $.post(url,param,function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/cdlbgl/addCdlbglSz';
		        	 $('#addOrUpdfrom').ajaxSubmit(function(r){
		         		document.ff.action="<%=path%>/cdlbgl/queryCdlbgl";
		         		document.ff.submit();
		         	});
<%--		    	    document.addOrUpdfrom.submit();--%>
	...
	function delCdlb(){
	    ...
	    if(confirm("您确定要删除选中的场地类别吗？") == true){
	    	var id = $("[name='ids']:checked");
	    	var ids="";
	    	$.each(id,function(){
	    		ids+=$(this).val() + ",";
	    	})
	    	var url = "<%=path%>/cdlbgl/delBefore";
	    	var zhi = {"ids":ids};
	    	$.post(url,zhi,function(result){
	    		if(result.mc == "" || result.mc == null){
	    			document.ff.action = "<%=path%>/cdlbgl/delCdlb";
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delCdlb();"><span>删&nbsp;&nbsp;除</span></a> </div></td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${cdlbglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${cdlbglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${cdlbglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${cdlbglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${cdlbglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${cdlbList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
      <c:if test="${not empty cdlbglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="cdlbglPage" onlyOneShow="true" showListNo="true" action="cdlbgl/queryCdlbgl"/>
      </c:if>
    </div>
    <div class="clr"></div> 
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
    <input type="hidden" name="pageCount"  value="${cdlbglPage.pageCount eq '0' ? '1' : cdlbglPage.pageCount}" />
    <input type="hidden" name="pageNo"  value="${cdlbglPage.pageNo eq '0' ? '1' : cdlbglPage.pageNo}" />
    <input type="hidden" name="pageSize"  value="${cdlbglPage.pageSize}" />
    <input type="hidden" name="dmmc"  value="${cdlbglPage.dmmc}" />
