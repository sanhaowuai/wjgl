...
$(document).ready(function(){
   $("#ff").baigoCheckall();
	...
	function querylike(){
		...
		document.queryform.action="<%=path%>/dqgl/queryDqgl";
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			var dm = $("#ff_dm").val();
			var zt = $("#zt").val();
			var url="<%=path%>/dqgl/queryByDmCount";
		    var param={'dm':dm,'zt':zt};
		    $.post(url,param,function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/dqgl/addDqglSz';
		        	$('#addOrUpdfrom').ajaxSubmit(function(r){
		        		document.ff.action="<%=path%>/dqgl/queryDqgl";
		        		document.ff.submit();
		        	});
			    }else{
				    alert("此代码已存在！");
	...
	function delDq(){
	    ...
	    if(confirm("您确定要删除选中的地区吗？") == true){
	        document.ff.action = "<%=path%>/dqgl/delDq";
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delDq();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${dqglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${dqglPage.pageCount}" />
  <input type="hidden" name="pageNo"  value="${dqglPage.pageNo}" />
  <input type="hidden" name="pageSize"  value="${dqglPage.pageSize}" />
  <input type="hidden" name="dmmc"  value="${dqglPage.dmmc}" />
	...
	<c:forEach items="${dqList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
      <c:if test="${not empty dqglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="dqglPage" onlyOneShow="true" showListNo="true" action="dqgl/queryDqgl"/>
      </c:if>
    </div>
    <div class="clr"></div>
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
    <input type="hidden" name="pageCount"  value="${dqglPage.pageCount eq '0' ? '1' : dqglPage.pageCount}" />
    <input type="hidden" name="pageNo"  value="${dqglPage.pageNo eq '0' ? '1' : dqglPage.pageNo}" />
    <input type="hidden" name="pageSize"  value="${dqglPage.pageSize}" />
    <input type="hidden" name="dmmc"  value="${dqglPage.dmmc}" />
