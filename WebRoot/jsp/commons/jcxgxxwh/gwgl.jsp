...
$(document).ready(function(){
   $("#ff").baigoCheckall();
	...
	function querylike(){
		...
		document.queryform.action="<%=path%>/gwgl/queryGwgl";
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			var dm = $("#ff_dm").val();
			var zt = $("#zt").val();
			var url="<%=path%>/gwgl/queryByDmCount";
		    ...
		    $.post(url,param,function(result){
		        ...
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/gwgl/addGwglSz';
		        	$('#addOrUpdfrom').ajaxSubmit(function(r){
		         		document.ff.action="<%=path%>/gwgl/queryGwgl";
		         		document.ff.submit();
		         	});
			    }else{
				    alert("此代码已存在！");
	...
	function delGw(){
	    ...
	    if(confirm("您确定要删除选中的岗位吗？") == true){
	        document.ff.action = "<%=path%>/gwgl/delGw";
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delGw();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${gwglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${gwglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${gwglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${gwglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${gwglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${gwList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
      <c:if test="${not empty gwglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="gwglPage" onlyOneShow="true" showListNo="true" action="gwgl/queryGwgl"/>
      </c:if>
    </div>
    <div class="clr"></div> 
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  ...
  <input type="hidden" name="pageCount"  value="${gwglPage.pageCount eq '0' ? '1' : gwglPage.pageCount}" />
  <input type="hidden" name="pageNo"  value="${gwglPage.pageNo eq '0' ? '1' : gwglPage.pageNo}" />
  <input type="hidden" name="pageSize"  value="${gwglPage.pageSize}" />
  <input type="hidden" name="dmmc"  value="${gwglPage.dmmc}" />
      ...
      <tr>
        ...
        <td align="left">
          <select name="gwlx" id="gwlx">
       	    <option value="-1" >--请选择岗位类型--</option>
       	    <c:forEach items="${gwglList}" var="gwglList">
              <option value="${gwglList.dm}">${gwglList.mc}</option>
       	    </c:forEach>
          </select>
          <span class="red">*</span>
        </td>
