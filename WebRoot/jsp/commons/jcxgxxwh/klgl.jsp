...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/klgl/queryKlgl";
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			...
			document.addOrUpdfrom.action="<%=path%>/klgl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/klgl/addKlglSz';
		        	$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.addOrUpdfrom.action="<%=path%>/klgl/queryKlgl";
	...
	function delKl(){
	    ...
	    if(confirm("您确定要删除选中的科类吗？") == true){
	        document.ff.action = "<%=path%>/klgl/delKl";
	        $("#ff").ajaxSubmit(function(r){
				document.ff.action="<%=path%>/klgl/queryKlgl";
				document.ff.submit();
			});
	    }
	}
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delKl();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0">
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${klglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${klglPage.pageCount}" />
		<input type="hidden" name="pageNo"  value="${klglPage.pageNo}" />
		<input type="hidden" name="pageSize"  value="${klglPage.pageSize}" />
		<input type="hidden" name="dmmc"  value="${klglPage.dmmc}" />
        <table width="100%"  cellpadding="0" cellspacing="0" style="border-collapse:collapse" class="bot_line">
          ...
          <c:forEach items="${klList}" var="map" varStatus="status">
    ...
    <div class="youtube01"> 
	  <c:if test="${not empty klglPage }">
	    <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="klglPage" onlyOneShow="true" showListNo="true" action="klgl/queryKlgl"/>
	  </c:if>
    </div>
    <div class="clr"></div> 
...
<div id="addOrUpd" style="display:none; width: 400px;border: 6px solid #ddd;-webkit-box-shadow: 0px 0px 5px #000;-moz-box-shadow: 0px 0px 5px #000;box-shadow: 0px 0px 5px #000;-webkit-border-radius: 10px;-moz-border-radius: 10px;border-radius: 10px;" >
  <form id="addOrUpdfrom" name="addOrUpdfrom" method="post">
	<input type="hidden" name="pageCount"  value="${klglPage.pageCount eq '0' ? '1' : klglPage.pageCount}" />
	<input type="hidden" name="pageNo"  value="${klglPage.pageNo eq '0' ? '1' : klglPage.pageNo}" />
	<input type="hidden" name="pageSize"  value="${klglPage.pageSize}" />
	<input type="hidden" name="dmmc"  value="${klglPage.dmmc}" />
