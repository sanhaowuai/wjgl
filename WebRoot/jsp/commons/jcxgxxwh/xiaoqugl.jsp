...
<script type="text/javascript">
	...
	function querylike(){
		document.queryform.action="<%=path%>/xiaoqugl/queryXiaoqugl";
	...
	function updDiv(dm){
		$("#zt").val("2");
		Validator.ClearForm('addOrUpdfrom');
		var url="<%=path%>/xiaoqugl/queryXiaoquByDm";
	    var param={'dm':dm};
	...
	function addOrUpd(){	
		if(Validator.Validate(document.getElementById('addOrUpdfrom'),3)){		
			...
			document.addOrUpdfrom.action="<%=path%>/xiaoqugl/queryByDmCount";
		    
			$("#addOrUpdfrom").ajaxSubmit(function(result){
	
		        if(result){
		        	document.addOrUpdfrom.action = '<%=path%>/xiaoqugl/addXiaoquglSz';
		        	$("#addOrUpdfrom").ajaxSubmit(function(r){
						document.addOrUpdfrom.action="<%=path%>/xiaoqugl/queryXiaoqugl";
	...
	function delXiaoqu(){
	    ...
	    if(confirm("您确定要删除选中的校区吗？") == true){
	    	var id = $("[name='ids']:checked");
	    	var ids="";
	    	$.each(id,function(){
	    		ids+=$(this).val() + ",";
	    	})
	    	var url = "<%=path%>/xiaoqugl/delBefore";
	    	var zhi = {"ids":ids};
	    	$.post(url,zhi,function(result){
	    		if(result.mc == "" || result.mc == null){
	    			document.ff.action = "<%=path%>/xiaoqugl/delXiaoqu";
	    			$("#ff").ajaxSubmit(function(r){
						document.ff.action="<%=path%>/xiaoqugl/queryXiaoqugl";
						document.ff.submit();
					});
	    		}else{
...
<%--    </div> --%>
    <div class="butsea">
      <table  border="0" align="left" cellpadding="0" cellspacing="0"  >
        <tr>
          <td height="35px">
            ...
            <div class="addlist_button  ml15"> <a href="javascript:delXiaoqu();"><span>删&nbsp;&nbsp;除</span></a> </div>
          </td>
        </tr>
      </table>
      <form id="queryform" name="queryform" method="post">
        <table  border="0" align="right" cellpadding="0" cellspacing="0"  >
          <tr>
            <td >名称：</td>
            <td><input type="text" name="dmmc" id="qf_dm" value="${xiaoquglPage.dmmc}"  /></td>
    ...
    <div class="list">
      <form name="ff" method="post" id="ff" >
        <input type="hidden" name="pageCount"  value="${xiaoquglPage.pageCount}" />
        <input type="hidden" name="pageNo"  value="${xiaoquglPage.pageNo}" />
        <input type="hidden" name="pageSize"  value="${xiaoquglPage.pageSize}" />
        <input type="hidden" name="dmmc"  value="${xiaoquglPage.dmmc}" />
    ...
    <div class="youtube01"> 
      <c:if test="${not empty xiaoquglPage }">
        <ff:page mhFrom="queryform" showReSize="true" listPageCount="10" field="xiaoquglPage" onlyOneShow="true" showListNo="true" action="xiaoqugl/queryXiaoqugl"/>
      </c:if>
    </div>
    <div class="clr"></div> 
    ...
    <div class="popup">
      ...
      <div class="clr"></div>
        <table id="divtab" width="100%"  cellpadding="0" cellspacing="0"  class="addlist" style="border-collapse:collapse">
          <tr>
            <th>代码：</th>
            <td align="left"><input type="text" name="dm" id="ff_dm"  dataType="Limit" min="1" max="15" msg="必填(15个字之内)"/>
              <span class="red">*</span>
            </td>
          </tr>
          <tr>
